...
namespace Wood.Core
{
    public class WoodCore
    {
        ...
        static readonly string BasePrefix;//= "http://*:8866/";
        IWebView webView;
        string Addr;
        string FullPrefix;
        static int Port;
        ...
        static WoodCore()
        {
            ...
            if (port >= 8870)
            {
                throw new Exception("端口8860-8869都被占用了！");
            }
        }
        ...
        void InvokeService(HttpListenerContext context, ServiceArgs args)
        {
            try
            {
                ...
                else
                {
                    ...
                    ReturnContentToClient(context, null);
                }

            }
            catch (Exception exp)
            {
                ReturnContentToClient(context, exp.Message);
            }
        }
        void ReturnContentToClient(HttpListenerContext context, object result)
        {
